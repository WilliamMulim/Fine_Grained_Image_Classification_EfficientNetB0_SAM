...
np.random.seed(123)
#prepare dataset from tensorflow-datasets (stanford_dogs , food101)
...
#hyperparameter to explore: l_rate,momentum,decay,optimizer,batch size, epoch
...
img_aug = Sequential(
    [
        preprocessing.RandomRotation(factor=0.2), #rotate images
        preprocessing.RandomTranslation(height_factor=0.125,width_factor=0.125), #shift images
        preprocessing.RandomFlip(), #flip images vertical and horizontally
...
def input_category (image,label):
    label = tf.one_hot(label,num_of_classes) #OneHot label by amount of classes
    return image,label


batch_size = 64 #hyperparameter 5
...
def model_build_no_sam(classes_num):
    ...
    optimizer = tf.keras.optimizers.Adam(learning_rate=1e-3) #hyperparameter 1 to 4 (optimizer,l_rate,momentum)
...
def model_build_sam(classes_num):
    ...
    model.trainable = False #freeze pretrained weight

    #create new top layers by adding pooling,BN, Dropout, and Dense(output) layer with Softmax activation
    x = layers.GlobalAveragePooling2D(name="avg_pool")(model.output)
    x = layers.BatchNormalization()(x)
    ...
    #create model from input layer to output
    model = tf.keras.Model(inputs,outputs_layer, name="EfficientNetB0")

    #unfreeze 20 layers from top so they can be trained, but leave BN layers frozen
    for layer in model.layers[-20:]:
        if not isinstance(layer,layers.BatchNormalization):
            layer.trainable = True
    ...
    optimizer = tf.keras.optimizers.Adam(learning_rate=1e-4) #hyperparameter 1 to 4
...
logger = tf.keras.callbacks.CSVLogger(
    'prediction_model_'+str(model_number)+'.csv', separator=',', append=True
)
#Stanford Dogs model
