...
dogs_train = dogs_train.batch(batch_size= batch_size, drop_remainder = False)

dogs_test = dogs_test.map(input_category)
dogs_test = dogs_test.batch(batch_size= batch_size, drop_remainder = False)

num_of_classes = food_classes
food_train = food_train.map(input_category)
food_train = food_train.batch(batch_size= batch_size, drop_remainder = False)

food_test = food_test.map(input_category)
food_test = food_test.batch(batch_size= batch_size, drop_remainder = False)
...
def model_build_sam(classes_num):
    ...
    optimizer = tf.keras.optimizers.Nadam(learning_rate=1e-4) #hyperparameter 1 to 4
