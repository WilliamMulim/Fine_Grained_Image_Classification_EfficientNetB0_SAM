...
logger = tf.keras.callbacks.CSVLogger(
    'prediction_model_'+str(model_number)+'_BSize32.csv', separator=',', append=True
)
#Stanford Dogs model

prediction_dog_sam_BSize32 = dogs_model.fit(dogs_train, epochs=epoch, validation_data=dogs_test, callbacks=[stopper,logger])

plot_hist(prediction_dog_sam_BSize32)
plt.savefig('dogs_prediction_SAM_BSize32')
plt.show()
plot_hist_loss(prediction_dog_sam_BSize32)
plt.savefig('dogs_prediction_SAM_loss_BSize32')
plt.show()

#Food101 model
model_number = 2
prediction_food_sam_BSize32 = food_model.fit(food_train, epochs=epoch, validation_data=food_test,callbacks=[stopper,logger])

plot_hist(prediction_food_sam_BSize32)
plt.savefig('food101_prediction_SAM_BSize32')
plt.show()
plot_hist_loss(prediction_food_sam_BSize32)
plt.savefig('food101_prediction_SAM_loss_BSize32')
plt.show()
