...
logger = tf.keras.callbacks.CSVLogger(
    'prediction_model_'+str(model_number)+'_RMSProp_Momentum.csv', separator=',', append=True
)


#Stanford Dogs model

prediction_dog_sam_RMSProp_Momentum = dogs_model.fit(dogs_train, epochs=epoch, validation_data=dogs_test, callbacks=[stopper,logger])

plot_hist(prediction_dog_sam_RMSProp_Momentum)
plt.savefig('dogs_prediction_SAM_RMSProp_Momentum')
plt.show()
plot_hist_loss(prediction_dog_sam_RMSProp_Momentum)
plt.savefig('dogs_prediction_SAM_loss_RMSProp_Momentum')
plt.show()

#Food101 model
model_number = 2
prediction_food_sam_RMSProp_Momentum = food_model.fit(food_train, epochs=epoch, validation_data=food_test,callbacks=[stopper,logger])

plot_hist(prediction_food_sam_RMSProp_Momentum)
plt.savefig('food101_prediction_SAM_RMSProp_Momentum')
plt.show()
plot_hist_loss(prediction_food_sam_RMSProp_Momentum)
plt.savefig('food101_prediction_SAM_loss_RMSProp_Momentum')
plt.show()
